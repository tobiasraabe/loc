...
def prepare_bioagel():
    ...
    df = pd.read_stata(ppj("IN_DATA", "bioagel.dta"), columns=RETAINED_COLUMNS_BIOAGEL)
    ...
    child_birth_month_dict = {
        k: MONTH_DICT[v]
        for v in MONTH_DICT
        for k in child_birth_month_values
        if k in v.casefold()
    }
    child_birth_month_dict.update(
        {"[-1] keine Angabe": np.nan, "[-2] trifft nicht zu": np.nan}
    )
    ...
    df.CHILD_BIRTH_MONTH.cat.set_categories(
        MONTH_DICT.values(), ordered=True, inplace=True
    )

    # Create PREGNANCY_START_YEAR and _MONTH
    pregstart = (
        df.PREGNANCY_START.str.split(" ", n=2, expand=True)
        .drop(0, axis=1)
        .fillna(value=np.nan)
    )
    pregstart = pregstart.rename(
        columns={1: "PREGNANCY_START_YEAR", 2: "PREGNANCY_START_MONTH"}
    )
    ...
    pregstart.PREGNANCY_START_MONTH.replace(correct_month_mapping, inplace=True)

    # Create PREGNANCY_END_YEAR and _MONTH
    pregend = (
        df.PREGNANCY_END.str.split(" ", n=2, expand=True)
        .drop(0, axis=1)
        .fillna(value=np.nan)
    )
    pregend = pregend.rename(
        columns={1: "PREGNANCY_END_YEAR", 2: "PREGNANCY_END_MONTH"}
    )
    # Rename monthly values by comparing short values to the keys of MONTH_DICT
    # and then the values of MONTH_DICT to map them to MONTH_DICT values
    ...
    df.PREGNANCY_START_YEAR = pd.to_numeric(df.PREGNANCY_START_YEAR, errors="coerce")
    df.PREGNANCY_END_MONTH = df.PREGNANCY_END_MONTH.astype("category")
    df.PREGNANCY_END_MONTH.cat.set_categories(
        MONTH_DICT.values(), ordered=True, inplace=True
    )
    df.PREGNANCY_END_YEAR = pd.to_numeric(df.PREGNANCY_END_YEAR, errors="coerce")
...
def create_preg(df):
    ...
    # pregnancy
    pregnancy_unplanned = {
        "[2] eher geplant": np.nan,
        "[1] eher ungeplant": True,
    ...
    df.loc[
        df.PREGNANCY_UNPLANNED.notnull(), "PREGNANCY_UNPLANNED_MONTH"
    ] = df.PREGNANCY_START_MONTH
    # Cast the variables to the appropriate types
    df.PREGNANCY_UNPLANNED_MONTH = df.PREGNANCY_UNPLANNED_MONTH.astype("category")
    ...
    preg = df[
        [
            "ID_HH",
            "ID_MOTHER",
            "PREGNANCY_UNPLANNED_MONTH",
            "PREGNANCY_START_YEAR",
            "MOTHER_PREGNANT_AT_PQ_YEAR",
        ]
    ].copy()
    # Restrict to events
    ...
    preg.loc[preg.duplicated(subset=["ID_MOTHER", "YEAR"])].to_pickle(
        ppj("OUT_DATA", "preg_siblings.pkl")
    )
...
def create_dis(df):
    ...
    df.loc[df.CHILD_DISORDER.notnull(), "CHILD_DISORDER_MONTH"] = df.CHILD_BIRTH_MONTH
    ...
    # handle this cases right now. Therefore, we drop the duplicates and save
    # them in another file to adjust the counts of events later.
    dis.loc[dis.duplicated(subset=["ID_MOTHER", "YEAR"])].to_pickle(
        ppj("OUT_DATA", "dis_siblings.pkl")
    )
