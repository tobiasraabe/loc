...
def create_leads(df):
    ...
    df_2010 = df_2010.merge(df_leads, on='ID', how='left',
                            suffixes=('', '_LEADS'))
    # Merge both periods
    df = df_2010.append(df_2015)

    # Fill NaNs due to merge with False
    df[[i for i in df if 'LEADS' in i]] = df.loc[:, [i for i in df if '_LEADS'
                                                     in i]].fillna(False)
...
def prepare_for_regression(df):
    ...
    df = df.merge(loc[['ID', 'YEAR', 'FIRST_FACTOR_DELTA']],
                  on=['ID', 'YEAR'], how='left')
