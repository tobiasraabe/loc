...
class FactorResults(object):
    ...
    def factor_score_params(self, method='bartlett'):
        ...
        if method == 'bartlett':
            s_mat = np.linalg.inv(
                L.T.dot(L / (uni[:, None]))).dot((L.T / uni)).T
            ...
            corr = self.model.corr
            corr_f = self._corr_factors()
            s_mat = np.linalg.inv(np.linalg.inv(
                corr_f) + L.T.dot(L / (uni[:, None])))
    ...
    def get_loadings_frame(self, style='display', sort_=True, threshold=0.3,
        ...
        if sort_ is True:
            ...
            loadings_df2['largest'] = np.abs(
                loadings_df.values[np.arange(n_f), high])
            loadings_df2.sort_values(by=['high', 'largest'], ascending=[
