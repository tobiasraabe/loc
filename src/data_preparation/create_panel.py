...
def calculate_time_difference_between_event_int(x, event, end_year_interview):
    try:
        ...
        end = datetime.datetime.strptime(f"{end_year} {end_month_number}", "%Y %m")
...
def clean_common_variables(df):
    ...
    df.loc[
        df.LEGALLY_HANDICAPPED_PERC_CHANGE == 100, "LEGALLY_HANDICAPPED_PERC_MONTH"
    ] = df.INT_MONTH
    df.LEGALLY_HANDICAPPED_PERC_MONTH = df.LEGALLY_HANDICAPPED_PERC_MONTH.astype(
        "category"
    )
    ...
    df["REASON_JOB_TERMINATED_SHIFTED"] = df.groupby(
        "ID"
    ).REASON_JOB_TERMINATED.transform(lambda x: x.shift(-1))
...
def extract_loc(df):
    ...
    loc = df.loc[
        df.YEAR.isin([2005, 2010, 2015]),
        ["ID", "YEAR", "AGE"] + [i for i in df if "LOC" in i],
    ].copy()
...
def merge_with_preg_dis(df):
    ...
    df.drop(
        [
            "ID_MOTHER_x",
            "ID_MOTHER_y",
            "MOTHER_PREGNANT_AT_PQ_YEAR_x",
            "MOTHER_PREGNANT_AT_PQ_YEAR_y",
        ],
        axis="columns",
        inplace=True,
    )
...
def clean_event_variables(df):
    ...
    for var in EVENT_VARIABLES:
        ...
        if var in [
            ...
            df[var + "_MONTH_PY_SHIFTED"] = df.groupby("ID")[var + "_MONTH_PY"].shift(
                -1
            )
    ...
    df_2005_2010.loc[
        df_2005_2010.REASON_JOB_TERMINATED.isnull()
        & df_2005_2010.REASON_JOB_TERMINATED_SHIFTED.isnull(),
        "LAST_JOB_ENDED_LIMITED_MONTH",
    ] = np.nan
    df_2010_2015["LAST_JOB_ENDED_LIMITED_MONTH"] = df_2010_2015.LAST_JOB_ENDED_MONTH
    df_2010_2015.loc[
        df_2010_2015.REASON_JOB_TERMINATED.isnull()
    ...
    int_month_2015 = df_2010_2015.loc[
        df_2010_2015.YEAR == 2015, ["ID", "INT_MONTH"]
    ].copy()

    df_2005_2010 = df_2005_2010.merge(
        int_month_2010, how="left", on="ID", suffixes=("", "_2010")
    )
    ...
    for var in EVENT_VARIABLES:
        ...
        df_2010_2015["EVENT_" + var + "_TIME_DIFF"] = df_2010_2015.groupby("ID")[
            "EVENT_" + var + "_TIME_DIFF"
        ].transform(lambda x: x.fillna(method="ffill").fillna(-1))
...
def drop_unused_columns_and_observations(df):
    ...
    unused_columns += ["REASON_JOB_TERMINATED", "REASON_JOB_TERMINATED_SHIFTED"]
    unused_columns += ["EVENT_" + i for i in EVENT_VARIABLES]
    unused_columns += ["ID_MOTHER", "MOTHER_PREGNANT_AT_PQ_YEAR"]
    unused_columns += [
        "EDUCATION_GROUPS_CASMIN",
        "EDUCATION_GROUPS_ISCED11",
        "YEARS_EDUCATION",
    ]
