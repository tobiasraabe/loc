...
def prepare_data():
    ...
    df = df[["ID", "YEAR", "EVENT_ANY"] + ["EVENT_" + i for i in EVENT_VARIABLES]]
    ...
    for i in EVENT_VARIABLES:
        table = pd.concat(
            [table, df.groupby("EVENT_" + i).FIRST_FACTOR_DELTA.mean()], axis=1
        )
    ...
    table = table.rename(index={False: "No event", True: "Event"}, columns=EVENT_MAP)
