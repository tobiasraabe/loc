...
def main(df):
    list_counts = [
        i
        for i in df
        if "EVENT" in i and "COUNT" in i and "PREVIOUS" not in i and "LIMITED" not in i
    ]
    ...
    model_1 = smf.ols(
        ...
        "EVENT_DEATH_CHILD) + C("
        "EVENT_DEATH_CHILD):EVENT_DEATH_CHILD_TIME_DIFF +  C("
        "EVENT_DEATH_FATHER) + C("
        "EVENT_DEATH_FATHER):EVENT_DEATH_FATHER_TIME_DIFF + C("
        "EVENT_DEATH_HH_PERSON) + C("
        "EVENT_DEATH_HH_PERSON):EVENT_DEATH_HH_PERSON_TIME_DIFF+ C("
        "EVENT_DEATH_MOTHER) + C("
        "EVENT_DEATH_MOTHER):EVENT_DEATH_MOTHER_TIME_DIFF + C("
        "EVENT_DEATH_PARTNER) + C("
        "EVENT_DEATH_PARTNER):EVENT_DEATH_PARTNER_TIME_DIFF + C("
        "MARITAL_STATUS):C(EVENT_DIVORCED) + C(EVENT_DIVORCED) + C("
        "EVENT_DIVORCED):EVENT_DIVORCED_TIME_DIFF + C(MARITAL_STATUS):C("
        "EVENT_DIVORCED):EVENT_DIVORCED_TIME_DIFF + C(MARITAL_STATUS) + C("
        "EVENT_HH_COMP_CHANGE) + C("
        "EVENT_HH_COMP_CHANGE):EVENT_HH_COMP_CHANGE_TIME_DIFF + C("
        "EVENT_LAST_JOB_ENDED):C(EMPLOYMENT_STATUS) + C("
        "EVENT_LAST_JOB_ENDED) + C("
        "EVENT_LAST_JOB_ENDED):EVENT_LAST_JOB_ENDED_TIME_DIFF + C("
        "EVENT_LAST_JOB_ENDED):EVENT_LAST_JOB_ENDED_TIME_DIFF:C("
        "EMPLOYMENT_STATUS) + C(EMPLOYMENT_STATUS) + C("
        "EVENT_LEGALLY_HANDICAPPED_PERC) + C("
        "EVENT_LEGALLY_HANDICAPPED_PERC"
        "):EVENT_LEGALLY_HANDICAPPED_PERC_TIME_DIFF + C("
        "EVENT_PREGNANCY_UNPLANNED) + C("
        "EVENT_PREGNANCY_UNPLANNED):EVENT_PREGNANCY_UNPLANNED_TIME_DIFF + C("
        "EVENT_SEPARATED):C(MARITAL_STATUS) + C(EVENT_SEPARATED) + C("
        "EVENT_SEPARATED):EVENT_SEPARATED_TIME_DIFF + C("
        "EVENT_SEPARATED):EVENT_SEPARATED_TIME_DIFF:C(MARITAL_STATUS)",
        df,
    )
    ...
    model_2 = smf.ols(
        ...
        "EVENT_HH_COMP_CHANGE) + C("
        "EVENT_HH_COMP_CHANGE):EVENT_HH_COMP_CHANGE_TIME_DIFF + C("
        "EVENT_LAST_JOB_ENDED_LIMITED):C(EMPLOYMENT_STATUS) + C("
        "EVENT_LAST_JOB_ENDED_LIMITED) + C("
        "EVENT_LAST_JOB_ENDED_LIMITED"
        "):EVENT_LAST_JOB_ENDED_LIMITED_TIME_DIFF + C("
        "EVENT_LAST_JOB_ENDED_LIMITED"
        "):EVENT_LAST_JOB_ENDED_LIMITED_TIME_DIFF:C(EMPLOYMENT_STATUS) + C("
        "EMPLOYMENT_STATUS) + C(EVENT_LEGALLY_HANDICAPPED_PERC) + C("
        "EVENT_LEGALLY_HANDICAPPED_PERC"
        "):EVENT_LEGALLY_HANDICAPPED_PERC_TIME_DIFF + C("
        "EVENT_PREGNANCY_UNPLANNED) + C("
        "EVENT_PREGNANCY_UNPLANNED):EVENT_PREGNANCY_UNPLANNED_TIME_DIFF",
        df,
    )
    ...
    model_4 = smf.ols(
        ...
        # 'EVENT_DEATH_CHILD_TIME_DIFF_GROUP) +  C(EVENT_DEATH_FATHER) + C('
        # 'EVENT_DEATH_FATHER):C(EVENT_DEATH_FATHER_TIME_DIFF_GROUP)+ C('
        # 'EVENT_DEATH_HH_PERSON) + C(EVENT_DEATH_HH_PERSON):C('
        # 'EVENT_DEATH_HH_PERSON_TIME_DIFF_GROUP) + C(EVENT_DEATH_MOTHER) + C('
        # 'EVENT_DEATH_MOTHER):C(EVENT_DEATH_MOTHER_TIME_DIFF_GROUP)+ C('
        # 'EVENT_DEATH_PARTNER) + C(EVENT_DEATH_PARTNER):C('
        # 'EVENT_DEATH_PARTNER_TIME_DIFF_GROUP)+ C(EVENT_HH_COMP_CHANGE) + C('
        # 'EVENT_HH_COMP_CHANGE):C(EVENT_HH_COMP_CHANGE_TIME_DIFF_GROUP) + C('
        # 'EVENT_LAST_JOB_ENDED_LIMITED):C(EMPLOYMENT_STATUS) + C('
        # 'EVENT_LAST_JOB_ENDED_LIMITED) + C(EVENT_LAST_JOB_ENDED_LIMITED):C('
        # 'EVENT_LAST_JOB_ENDED_LIMITED_TIME_DIFF_GROUP) + C('
        # 'EVENT_LAST_JOB_ENDED_LIMITED):C('
        # 'EVENT_LAST_JOB_ENDED_LIMITED_TIME_DIFF_GROUP):C(EMPLOYMENT_STATUS) '
        # '+ C(EMPLOYMENT_STATUS)+ C(EVENT_LEGALLY_HANDICAPPED_PERC) + C('
        # 'EVENT_LEGALLY_HANDICAPPED_PERC):C('
        # 'EVENT_LEGALLY_HANDICAPPED_PERC_TIME_DIFF_GROUP) + C('
        # 'EVENT_PREGNANCY_UNPLANNED) + C(EVENT_PREGNANCY_UNPLANNED):C('
        # 'EVENT_PREGNANCY_UNPLANNED_TIME_DIFF_GROUP)'
        ,
        df,
    )
    res_4 = model_4.fit(cov_type="cluster", cov_kwds={"groups": df["ID"]}, use_t=True)
    ...
    reg_table_3 = " \n ".join(reg_table_3[0:17] + reg_table_3[35:39] + reg_table_3[47:])
    ...
    with open(ppj("OUT_TABLES", "reg_table_4.tex"), "w") as table:
        table.write(res_4.summary().as_latex())
