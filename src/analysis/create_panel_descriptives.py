...
DESCRIPTIVES = {
    ...
    "EDUCATION_GROUPS_ISCED97_[2] general elementary": "General elementary",
    "EDUCATION_GROUPS_ISCED97_[3] middle vocational": "Middle vocational",
    "EDUCATION_GROUPS_ISCED97_[4] vocational + Abi": "Vocational + Abi",
    "EDUCATION_GROUPS_ISCED97_[5] higher vocational": "Higher vocational",
    "EDUCATION_GROUPS_ISCED97_[6] higher education": "Higher education",
}
...
def prepare_data():
    # Load data
    df_2005_2010 = pd.read_pickle(ppj("OUT_DATA", "panel_2005_2010_inspection.pkl"))
    df_2005_2010 = df_2005_2010[RETAINED_COLUMNS]
    df_2010_2015 = pd.read_pickle(ppj("OUT_DATA", "panel_2010_2015_inspection.pkl"))
    ...
    df["EVENT_ANY"] = df[["EVENT_" + i for i in EVENT_VARIABLES]].any(axis="columns")
...
def generate_numbers(df):
    ...
    gen_num["Number of obs in both periods"] = (
        gen_num["Number of obs"] - gen_num["Number of unique IDs"]
    )
    gen_num["Number of obs in 2005-2010"] = df[df.YEAR == 2005].shape[0]
    gen_num["Number of obs in 2010-2015"] = df[df.YEAR == 2010].shape[0]

    with open(ppj("OUT_TABLES", "panel_descriptives_numbers.json"), "w") as file:
...
def generate_descriptive_statistics(df, event):
    ...
    df = pd.concat([pd.get_dummies(df[CONTROLS]), df[event_col]], axis=1)
    # Save columns names
    col_names = list(df.drop(event_col, axis=1).columns)
    # Create table
    table = df.groupby(event_col)[col_names].mean().T
    table.drop(["GENDER_Male", "MARITAL_STATUS_Single"], axis="rows", inplace=True)
    table = table.rename(index=DESCRIPTIVES)

    event_fn = event.replace("_", "-").lower()
    with open(
        ppj("OUT_TABLES", f"tab-panel-descriptive-statistics-{event_fn}.tex"), "w"
    ) as f:
        ...
        for variable, [event_false, event_true] in table.iterrows():
            if variable in ["In school"]:
                ...
                f.write("\t\\rowgroup{\\textit{School degree}} \\\\\n")
            f.write(f"\t{variable} & {event_false:.3f} & {event_true:.3f} \\\\\n")
